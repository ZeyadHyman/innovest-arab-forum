...
const Landing = () => {
  ...
  return (
    <div className="relative pt-16 sm:pt-20 lg:pt-24 min-h-screen">
      ...
      <div className="relative z-10">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 lg:py-20">
          <div className="max-w-4xl sm:max-w-5xl lg:max-w-6xl mx-auto text-center">
            ...
            <p
              data-aos="fade-right"
              className={`text-2xl sm:text-3xl lg:text-4xl font-bold text-white/90 mb-3 ${isArabic ? "ar-font" : ""}`}
            >
              {t("hero.welcome")}
            </p>
            ...
            <h1
              data-aos="fade-left"
              className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-gold mb-4 ${isArabic ? "ar-font" : "org-font tracking-widest"}`}
            >
              {t("hero.title")}
            </h1>
            ...
            <div className="flex items-center justify-center gap-2 text-gold mb-6 sm:mb-8">
              <TbSchoolBell className="text-lg sm:text-xl" />
              <span className="text-base sm:text-lg font-medium tracking-wider">{t("hero.edition")}</span>
            </div>

            {/* Countdown Timer */}
            <div className="flex justify-center gap-2 sm:gap-3 lg:gap-4 mb-10 sm:mb-12">
              {Object.entries(timeLeft).map(([unit, { value, unit: formattedUnit }]) => (
                <div
                  key={unit}
                  data-aos="fade-up"
                  data-aos-delay={unit === "days" ? 0 : unit === "hours" ? 100 : unit === "minutes" ? 200 : 300}
                  className="flex flex-col items-center justify-center bg-white/10 backdrop-blur-sm rounded-xl p-3 sm:p-4 min-w-[70px] sm:min-w-[80px] lg:min-w-[100px] border border-white/20"
                >
                  <span className="text-xl sm:text-2xl lg:text-3xl font-bold text-white">{value}</span>
                  <span className="text-xs sm:text-sm lg:text-base text-white/80">{formattedUnit}</span>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 lg:gap-6 mb-6 sm:mb-8">
              ...
              >
                <div className="flex items-center gap-2 text-gold mb-3 sm:mb-4 group-hover:text-gold/90 transition-colors duration-300">
                  <IoMdTime className="text-lg sm:text-xl group-hover:rotate-12 transition-transform duration-300" />
                  <span className="text-base sm:text-lg font-medium">{t("hero.openingDate")}</span>
                </div>
                <a
                ...
                >
                  <FaMapMarkerAlt className="text-gold text-lg sm:text-xl group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
                  <span className="text-sm sm:text-base text-center">{t("hero.openingVenue")}</span>
                </a>
              </div>
              ...
              >
                <div className="flex items-center gap-2 text-gold mb-3 sm:mb-4 group-hover:text-gold/90 transition-colors duration-300">
                  <IoMdTime className="text-lg sm:text-xl group-hover:rotate-12 transition-transform duration-300" />
                  <span className="text-base sm:text-lg font-medium">{t("hero.closingDate")}</span>
                </div>
                <a
                ...
                >
                  <FaMapMarkerAlt className="text-gold text-lg sm:text-xl group-hover:scale-110 group-hover:rotate-12 transition-transform duration-300" />
                  <span className="text-sm sm:text-base text-center">{t("hero.closingVenue")}</span>
                </a>
              </div>
            </div>
            ...
            <div className="flex justify-center">
              ...
              >
                {t("navbar.register")}
                <FaTicketAlt className="text-lg sm:text-xl transform group-hover:rotate-12 transition-transform duration-300" />
              </button>
            </div>
          </div>
        </div>
